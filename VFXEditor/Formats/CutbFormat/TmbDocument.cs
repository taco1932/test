...
namespace VfxEditor.CutbFormat {
    public class CutbDocument : FileManagerDocument<CutbFile, WorkspaceMetaBasic> {
        ...
        public CutbDocument( CutbManager manager, string writeLocation, string localPath, WorkspaceMetaBasic data ) : this( manager, writeLocation ) {
            LoadWorkspace( localPath, data.RelativeLocation, data.Name, data.Source, data.Replace, data.Disabled );
            AnimationId = CutbSpawn.GetIdFromCutbPath( ReplacePath );
        }
