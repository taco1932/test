...
namespace VfxEditor.CutbFormat.Utils {
    public class CutbReader : ParsingReader {
        private readonly Dictionary<int, CutbItemWithId> ItemsWithId = [];

        public CutbReader( BinaryReader reader ) : base( reader ) { }

        public void ParseItem( CutbFile file, List<Tmac> actors, List<Tmtr> tracks, List<TmbEntry> entries, List<Tmfc> tmfcs, ref VerifiedStatus verified ) {
            ...
            if( magic == "CTEX" ) {
                var actor = new Ctex( file, this );
                actors.Add( actor );
                entry = actor;
            }
            else if( magic == "CTTL" ) {
                var track = new CTTL( file, this );
                tracks.Add( track );
                entry = track;
            }
            else if( magic == "TMFC" ) {
